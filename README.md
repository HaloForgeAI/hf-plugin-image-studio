...
npm run hf -- plugin install local /path/to/hf-plugin-image-studio/dist/package/dev.haloforge.image-studio-0.1.6.hfpkg --json
...
gh workflow run "Plugin Release" \
  ...
  -f release_tag=v0.1.6 \
  -f release_name="Image Studio v0.1.6" \
  -f source=official
```
